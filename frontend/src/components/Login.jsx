...
import { useLocation } from 'react-router-dom'; // <-- Router hook use karenge URL check karne ke liye
import { loginUser, registerUser } from '../api/client';

export default function Login({ onLoginSuccess }) {
  const location = useLocation();
  
  // Dynamic initialization based on URL query param
  const isSignupQuery = new URLSearchParams(location.search).get('mode') === 'signup';
  const [isLogin, setIsLogin] = useState(!isSignupQuery);
  
  const [formData, setFormData] = useState({ username: '', email: '', password: '' });
  const [error, setError] = useState('');

  // Sync state if user switches between login and get started tabs
  useEffect(() => {
    const checkSignup = new URLSearchParams(location.search).get('mode') === 'signup';
    setIsLogin(!checkSignup);
  }, [location.search]);
